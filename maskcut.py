...
from utils_function.utils import resize_pil,detect_box,IoU
...
# modify the feature map feats by masking out certain parts of it based on a painting matrix and a mask.
# mask out parts of the affinity matrix to focus on specific areas of the image, enhancing the segmentation process 
# by excluding certain patches that are not of interest.
def get_masked_affinity_matrix(painting, feats, mask, ps):
    # mask out affinity matrix based on the painting matrix
    # feats : tensor shape [D,N]
    # painting matrix : tensor, represents which areas of the feature map should be masked out. [H,W]
    # mask : tensor, binary matrix. [H//patch_size,W//patch_size], 
    # ps : H//patch_size = W//patch_size
    dim, num_patch = feats.size()[0], feats.size()[1]; # D,N
    painting = painting + mask.unsqueeze(0);# [H//ps,W//ps] + [1,H//ps,W//ps] -> [1,H,W] 
    
    # binarize the painting matrix
    painting[painting > 0] = 1; # will be masked. #[1,H,W]
    painting[painting <= 0] = 0 # will be retained. #[1,H,W]
    
    feats = feats.clone().view(dim, ps, ps); # [D,N]-> [D,ps,ps]
    # inverts the painting matrix regions that were marked as 1 (foreground will be masked out) become 0 
    # and those marked as 0 (areas to be retained) become 1.
...
def maskcut_forward(feats, dims, scales, init_image_size, tau=0, N=3, cpu=False):
    ...
    return seeds, bipartitions, eigvecs
...
def maskcut(img_path, backbone,patch_size, tau, N=1, fixed_size=480, cpu=False) :
    ...
    seeds, bipartition, eigvec = maskcut_forward(feat, [feat_h, feat_w], [patch_size, patch_size], [h,w], tau, N=N, cpu=cpu)
...
# Ensures that a given contour (a sequence of points representing a boundary) derived from the segmentation masks 
# is properly closed before further processing or evaluation.
def close_contour(contour):
    # check that first and last point are the same point
    if not np.array_equal(contour[0], contour[-1]):
        # if not, just make it close
