...
# 008998.jpg
# python3 main_visualize.py --img-path ../data/VOC/2007/test/JPEGImages/000011.jpg --model-weight ./outputs/v5/Conv/FOUND-DUTS-TR-vit_small8/decoder_weights_niter500.pt --output-dir ./outputs/vis_con/
# python3 main_visualize.py --img-path ../data/VOC/2007/test/JPEGImages/004617.jpg --model-weight ./outputs/v5/Conv/FOUND-DUTS-TR-vit_small8/decoder_weights_niter500.pt --output-dir ./outputs/vis_con/
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="configs/kfound_DUTS-TR.yaml",
    )
    parser.add_argument(
    ...
    masks, _ = model.get_bkg_pseudo_labels_batch(
                            att=att,
                            shape_f=shape_f,
                            data=(inputs,img_t,img_t,args.img_path),
                            shape=preds.shape[-2:],
                        )
    ...
    #print(masks[None,None,...].shape,torch.unique(masks));
    pseudo_mask_up = F.interpolate(masks[None,None,...].float(),scale_factor=model.vit_patch_size,mode="bilinear",align_corners=False)[...,:h,:w];
    preds_up = (
    ...
    plt.imshow(img);
    plt.imshow(pseudo_mask_up.cpu().squeeze().numpy(),'gray', interpolation='none', alpha=0.5);
    plt.axis("off");
    img_name = args.img_path
    img_name = img_name.split('/')[-1].split('.')[0]
    plt.savefig(os.path.join(args.output_dir, f'{img_name}-pseudo_label.png'), bbox_inches='tight', pad_inches=0)
    plt.close()