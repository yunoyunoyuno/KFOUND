...
def evaluate_saliency(
    ...
    metrics_res = {}
    metrics_res_bs = {}
    # Disable tqdm for cleaner output - use simple iteration counter instead
    valbar = enumerate(valloader, 0)
    total = len(valloader)
    for i, data in valbar:
        ...
        if apply_bilateral:
            ...
            if s == "dcrf": 
                preds_bs = batch_apply_dense_crf(data,original_up.detach(),get_all_cc=get_all_cc)
            elif s == "bs":      
        ...
        if (i + 1) % 10 == 0 or (i + 1) == total:
            print(f"\r[{i+1}/{total}] {bar_str}", end='', flush=True)
    ...
    result_metrics = {
        'f_max': metrics_res['f_maxs'].avg,
        'iou': metrics_res['ious'].avg,
        'pixel_acc': metrics_res['pixel_accs'].avg
    }
    
    if apply_bilateral:
        result_metrics['f_max_bs'] = metrics_res_bs['f_maxs'].avg
        result_metrics['iou_bs'] = metrics_res_bs['ious'].avg
        result_metrics['pixel_acc_bs'] = metrics_res_bs['pixel_accs'].avg
