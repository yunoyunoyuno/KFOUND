...
class FoundDataset(Dataset):
    def __init__(
        ...
        # if ImageNet dataset
            ...
            if("20k" not in self.name):
                year = int("20"+self.name[-2:]);
                annFile=f'./data/coco20k/annotations/instances_{dataset_set}{str(year)}.json'
            else:
                year = "2014"
                if dataset_set == "val":
                    year = "2017"
                annFile = f"./data/coco20k/annotations/instances_{dataset_set}{str(year)}.json"
            self.dataset_set = dataset_set;
            print("annFile",annFile,type(annFile))
            self.cocoGt=COCO(annFile)
            self.img_ids = list(sorted(self.cocoGt.getImgIds()))
            #print("|iamges|",len(self.img_ids))

            if ("20k" in self.name and dataset_set == "train"):
                valid_img_ids = [];
                #print("F",os.getcwd())
                with open("./coco_20k_filenames.txt","r") as f:
                    filename_filter = [line.strip() for line in f.readlines()]
                for img_id in self.img_ids:
                    img_info = self.cocoGt.loadImgs(img_id)[0];
                    img_path = os.path.join(dataset_set + year, img_info["file_name"]);
                    if img_path in filename_filter:
                        valid_img_ids.append(img_id);
                self.img_ids = valid_img_ids;
                
            self.img_dir = f'./data/coco20k/images/{dataset_set}/'
